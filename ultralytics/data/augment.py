...
class AdaptiveAugmentPolicy:
    ...
    def __init__(self, game_intensity=0.5, fisheye_strength=(-0.3, 0.3),
                 perspective_shift=0.3, p=0.7, soft_routing=True):
    ...
    def _compute_routing_probs(self, scores, temperature=0.1):
        """Compute soft routing probabilities s_t from scene scores (Section 3.5).

        Uses temperature-scaled softmax: lower temperature → sharper
        distribution (more selective routing).  Default 0.1 gives a
        reasonable balance between exploration and exploitation.

        Returns a dict {scene_type: probability} that sums to 1.0.
        """
        keys = list(scores.keys())
        vals = np.array([scores[k] for k in keys], dtype=np.float64)
        vals = (vals - vals.max()) / max(temperature, 1e-6)
        probs = np.exp(vals)
        probs /= probs.sum()
    ...
    def __call__(self, labels):
        ...
        When ``soft_routing=True`` (Section 3.5), a single augmentation is
        selected via categorical sampling from the routing probability
        distribution s_t, making the detector robust to scene
        misclassification.
        ...
        # Fast Mode (Section 4.4): skip scene classifier
        if os.environ.get("YOLOV14_FAST", "0") == "1":
        ...
        if self.soft_routing:
            # --- Soft routing (Section 3.5): categorical sampling ---
            routing_probs = self._compute_routing_probs(profile)
            labels["scene_probs"] = routing_probs

            # Select ONE augmentation via categorical sampling
            aug_types = list(routing_probs.keys())
            aug_weights = [routing_probs[k] for k in aug_types]
            aug_type = random.choices(aug_types, weights=aug_weights)[0]

            if aug_type == "game":
                labels = self.game_aug(labels)
                labels["scene_type"] = "game"
            elif aug_type == "low_light":
                alpha = random.uniform(1.2, 1.8)
                beta = random.uniform(10, 30)
                img = labels.get("img", img)
            ...
            # --- Hard routing (original behavior) ---
